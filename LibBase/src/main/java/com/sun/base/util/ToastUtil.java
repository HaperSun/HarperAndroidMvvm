...
public final class ToastUtil {
    ...
    private static int yOffset         = (int) (64 * AppUtil.getCtx().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) AppUtil.getCtx().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(AppUtil.getCtx().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(AppUtil.getCtx().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(AppUtil.getCtx());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(AppUtil.getCtx(), spannableString, duration);
            } else {
                sToast = Toast.makeText(AppUtil.getCtx(), text, duration);
            }
        }

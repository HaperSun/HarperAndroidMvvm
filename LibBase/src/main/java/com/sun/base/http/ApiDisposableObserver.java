...
import com.sun.base.base.AppManager;
 ...
 * @author: Harper
 * @date: 2022/9/9
 * @note: 统一的Code封装处理。该类仅供参考，实际业务逻辑, 根据需求来定义，
 */
public abstract class ApiDisposableObserver<T> extends DisposableObserver<T> {
    ...
    public void onError(Throwable e) {
        ...
        if (e instanceof ResponseThrowable) {
            ...
            ToastUtil.showShort(rError.message);
            return;
        }
        //其他全部甩锅网络异常
    ...
    public void onStart() {
        super.onStart();
        ToastUtil.showShort("http is start");
        // if  NetworkAvailable no !   must to call onCompleted
        if (!NetworkUtil.isNetworkAvailable(AppUtil.getCtx())) {
            KLog.d("无网络，读取缓存数据");
            onComplete();
        }
    }
    ...
    public void onNext(Object o) {
        ...
        switch (baseResponse.getCode()) {
            ...
            case CodeRule.CODE_300:
                //请求失败，不打印Message
                KLog.e("请求失败");
                ToastUtil.showShort("错误代码:", baseResponse.getCode());
                break;
